...
function checkCards() {
    ...
    if (line.length == 0){
        ...
        $('#ModalMsg').text("Skchk Error: Cards form empty.");
        playError();
        return;
    }
    if (sk_live.length == 0) {
        ...
        $('#ModalMsg').text("Skchk Error: Secret Key (SK) form empty.");
        playError();
        return;
    }
    if (sk_live.indexOf('sk_live')==-1){
        $('#Modal').modal('show');
        $('#ModalTitle').text("SK Checker");
        $('#ModalMsg').text("Skchk Error: Secret Key (SK) provided is invalid.");
        playError();
        return;
    }
	playClick();
    ...
    check_line.forEach(function(value, index) {
        setTimeout(
            function(){
                $.ajax({
                    url: apis + '?cc_info=' + value + '&sk=' + sk_live + '&referrer=had3s',
                    type: 'GET',
                    async: true,
                    success: function(results) {
                        ...
                        remove_line();
                        if(results.match("Approved credit card information | CVV LIVE")) {
                            $("#approved_counter_cvv").text(count_live_cvv);
                            playSuccess();
                        }else if(results.match("Correct card info but wrong CVV | CCN LIVE")){
                            $("#approved_counter_ccn").text(count_live_ccn);
                            playSuccess();
                        }else if(results.match("Error. wrong credit card information | DEAD")){
...
function checkSK(){
    ...
    $('#Modal').modal('show');
    $('#ModalTitle').text("SK Checker");
    $('#ModalMsg').text("Skchk Initailizing SK Checker Script...");
    setTimeout(
        function(){
            $.ajax({
            url: 'skcheck.php?sk=' + sk_live + '&referrer=Skchk',
            type: 'GET',
            async: true,
            beforeSend: function () {
                $('#ModalMsg').text("Skchk Checking SK provided...");
            },
            success: function(data){
                if (data.match("LIVE")) {
                    $('#ModalMsg').text("Sucess! SK provided is LIVE.");
                    playSuccess();
                }else if(data.match("DEAD")){
                    $('#ModalMsg').text("Error! SK provided is DEAD. Create new SK at stripe.com");
                    playError();
                }
            }
        });
